...
HH_HHMAXINPCHAN = 8  # max num of physical input channels
        ...
        **kwargs
        ...
        for name, ckwargs in children.items():
            if name == "detector0":
                if "shutter0" in dependencies:
                    ...
                    shutter_name = None
                self._detectors[name] = PH300RawDetector(
                    channel=0,
                    parent=self,
                ...
                raise ValueError(
                    "Child %s not recognized, should be detector0 or detector1."
                )
        for name, comp in dependencies.items():
            if name == "shutter0":
                ...
                raise ValueError(
                    "Dependency %s not recognized, should be shutter0 or shutter1."
                )
...
class HH400(model.Detector):
    ...
    def GetHardwareDebugInfo(self):
        """
        Use this call to obtain debug information for support enquiries if you detect HH_FLAG_SYSERROR or HH_ERROR_STATUS_FAIL.
        """
